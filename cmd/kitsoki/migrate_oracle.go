// migrate_oracle.go — implements `kitsoki migrate-oracle <app.yaml>` (Phase 6).
//
// Walks the YAML tree of a kitsoki app.yaml (or testdata/apps/*/app.yaml),
// finds every invoke: host.oracle.ask_with_mcp and host.oracle.talk call,
// classifies each one using the §6 decision table, and rewrites the
// invoke: verb in place. Comments and formatting outside the rewritten spans
// are preserved (byte-surgery approach, same pattern as kitsoki-migrate-templates).
//
// Classification rules (§6 of oracle-split proposal):
//
//  1. chat_id: set → converse
//  2. host.oracle.talk → converse (warn; alias already present)
...
//  8. ambiguous (can't determine) → flagged in .migrate-oracle.todo
//
// Rewrite modes:
//
...
//
// Ambiguous cases: written to <file>.migrate-oracle.todo (Markdown).
package main
...
// migrateOracleCmd returns the `kitsoki migrate-oracle` command.
func migrateOracleCmd() *cobra.Command {
	var (
		dryRun          bool
	...
	cmd := &cobra.Command{
		Use:   "migrate-oracle <app.yaml>",
		Short: "Rewrite host.oracle.ask_with_mcp/talk calls to the new oracle verbs (Phase 6)",
		Long: `Walk the YAML tree of <app.yaml> (or a directory containing app.yaml files)
and rewrite every host.oracle.ask_with_mcp and host.oracle.talk effect to the
appropriate new verb: extract, decide, ask, task, or converse.

Classification rules (oracle-split proposal §6):
  chat_id: set                          → converse
  host.oracle.talk                      → converse
  ...
  ambiguous                             → flagged in <file>.migrate-oracle.todo

Flags:
  --dry-run           print unified diff; do not write
  --print-classified  list each call's classification before rewriting

Ambiguous calls are written to a Markdown .migrate-oracle.todo file next to
the input YAML for human review.`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			path := args[0]
			return runMigrateOracle(cmd, path, dryRun, printClassified)
		},
	}
...
// oracleCallClassification names the six output verbs plus an ambiguous bucket.
type oracleCallClassification string

const (
	classExtract   oracleCallClassification = "extract"
	classDecide    oracleCallClassification = "decide"
	classAsk       oracleCallClassification = "ask"
	classTask      oracleCallClassification = "task"
	classConverse  oracleCallClassification = "converse"
	classAmbiguous oracleCallClassification = "ambiguous"
)

// oracleCallSite records one host.oracle.ask_with_mcp or host.oracle.talk
// call found in the YAML tree.
type oracleCallSite struct {
	// InvokeByteStart / InvokeByteEnd is the byte range of the current verb
	// string value (e.g. "host.oracle.ask_with_mcp") in the source. The
	...
	Classification  oracleCallClassification
	Ambiguous       bool
...
// runMigrateOracle processes one app.yaml file (or all app.yaml files under
// a directory, up to two levels deep).
func runMigrateOracle(cmd *cobra.Command, path string, dryRun, printClassified bool) error {
	fi, err := os.Stat(path)
	if err != nil {
		return fmt.Errorf("migrate-oracle: stat %q: %w", path, err)
	}
	...
	if fi.IsDir() {
		...
		if readErr != nil {
			return fmt.Errorf("migrate-oracle: readdir %q: %w", path, readErr)
		}
	...
	if len(yamlFiles) == 0 {
		fmt.Fprintf(cmd.OutOrStdout(), "migrate-oracle: no app.yaml files found under %q\n", path)
		return nil
	}

	for _, f := range yamlFiles {
		if err := migrateOracleFile(cmd, f, dryRun, printClassified); err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "migrate-oracle: %s: %v\n", f, err)
		}
	}
	return nil
}

// migrateOracleFile processes one YAML file.
func migrateOracleFile(cmd *cobra.Command, path string, dryRun, printClassified bool) error {
	src, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("read %q: %w", path, err)
	}

	sites, err := findOracleCallSites(path, src)
	if err != nil {
		return err
	}
	if len(sites) == 0 {
	...
	// Classify each site.
	for i := range sites {
		classifyOracleCallSite(&sites[i])
	}
	...
	var ambiguousSites []oracleCallSite
	for _, s := range sites {
		if s.Ambiguous {
	...
	for _, s := range sites {
		...
		newVerb := newOracleVerb(s.OriginalVerb, s.Classification)
		if newVerb == "" {
			continue
		}
		// Verb replacement.
	...
	if dryRun {
		...
		fmt.Fprintf(cmd.OutOrStdout(), "migrate-oracle: rewrote %s (%d call(s))\n", path, len(sites)-len(ambiguousSites))
	}

	// Write the .migrate-oracle.todo file.
	if len(ambiguousSites) > 0 {
		todoPath := path + ".migrate-oracle.todo"
		if err := writeMigrateOracleTodo(todoPath, path, ambiguousSites); err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "migrate-oracle: write todo %q: %v\n", todoPath, err)
		} else {
			fmt.Fprintf(cmd.OutOrStdout(), "migrate-oracle: wrote %s (%d ambiguous call(s) require review)\n", todoPath, len(ambiguousSites))
		}
	}
...
// findOracleCallSites parses the YAML and locates every invoke:
// host.oracle.ask_with_mcp / host.oracle.talk call.
func findOracleCallSites(name string, src []byte) ([]oracleCallSite, error) {
	f, err := parser.ParseBytes(src, parser.ParseComments)
	if err != nil {
		return nil, fmt.Errorf("%s: parse YAML: %w", name, err)
	}
	lineIdx := buildOracleLineIndex(src)
	var sites []oracleCallSite
	for _, doc := range f.Docs {
		walkOracleAST(doc, "", src, lineIdx, &sites)
	}
	return sites, nil
}

// buildOracleLineIndex builds a byte-offset → line start index.
func buildOracleLineIndex(src []byte) []int {
	idx := []int{0}
	for i, b := range src {
		if b == '\n' {
			idx = append(idx, i+1)
...
// walkOracleAST visits the AST tree and records every oracle call site.
func walkOracleAST(n ast.Node, path string, src []byte, lineIdx []int, sites *[]oracleCallSite) {
	walkOracleASTWithParent(n, nil, path, src, lineIdx, sites)
}

// walkOracleASTWithParent is the recursive implementation; parent is the
// enclosing *ast.MappingNode when n is a *ast.MappingValueNode, so the
// site extractor can walk siblings for the with: block.
func walkOracleASTWithParent(n ast.Node, parent *ast.MappingNode, path string, src []byte, lineIdx []int, sites *[]oracleCallSite) {
	if n == nil {
		return
	}
	switch v := n.(type) {
	case *ast.DocumentNode:
		if v.Body != nil {
			walkOracleASTWithParent(v.Body, nil, path, src, lineIdx, sites)
		}
	case *ast.MappingNode:
		for _, mv := range v.Values {
			walkOracleASTWithParent(mv, v, path, src, lineIdx, sites)
		}
	case *ast.MappingValueNode:
		key := oracleMapKeyString(v.Key)
		childPath := path + "/" + key
		if key == "invoke" {
			if sv, ok := v.Value.(*ast.StringNode); ok {
				verb := sv.Value
				if verb == "host.oracle.ask_with_mcp" || verb == "host.oracle.talk" {
					site := extractOracleCallSite(path, verb, v, parent, src, lineIdx)
					if site != nil {
		...
		walkOracleASTWithParent(v.Value, nil, childPath, src, lineIdx, sites)
	case *ast.SequenceNode:
		for i, item := range v.Values {
			walkOracleASTWithParent(item, nil, fmt.Sprintf("%s[%d]", path, i), src, lineIdx, sites)
		}
	}
}

// extractOracleCallSite builds an oracleCallSite from the mapping value node
...
func extractOracleCallSite(path, verb string, invokeNode *ast.MappingValueNode, parentMapping *ast.MappingNode, src []byte, lineIdx []int) *oracleCallSite {
...
func populateSiteFromWithBlock(site *oracleCallSite, parentMapping *ast.MappingNode) {
	var withNode ast.Node
	for _, mv := range parentMapping.Values {
		switch oracleMapKeyString(mv.Key) {
		case "with":
			withNode = mv.Value
		}
	}
		...
		if k == "validator" {
			if vm, vmOK := wv.Value.(*ast.MappingNode); vmOK {
				for _, vv := range vm.Values {
					if oracleMapKeyString(vv.Key) == "post_cmd" && !isNullNode(vv.Value) {
	...
	for _, wv := range withMapping.Values {
		if oracleMapKeyString(wv.Key) == "prompt" || oracleMapKeyString(wv.Key) == "prompt_path" {
...
func extractWithBlockForTask(src []byte, lineIdx []int, invokeKeyIndent, invokeLineNo int, site *oracleCallSite) {
...
//	      key: value
//	      ...
func buildTaskWithBlock(s oracleCallSite) string {
...
// classifyOracleCallSite applies the §6 decision table.
func classifyOracleCallSite(s *oracleCallSite) {
	// Rule 1: host.oracle.talk → converse (warn; alias already present from Phase 7)
	if s.OriginalVerb == "host.oracle.talk" {
...
// newOracleVerb maps an original verb and a classification to the new verb string.
func newOracleVerb(originalVerb string, cls oracleCallClassification) string {
	if cls == classAmbiguous {
		return ""
	}
	return "host.oracle." + string(cls)
}

// writeMigrateOracleTodo writes a Markdown file listing ambiguous calls.
func writeMigrateOracleTodo(todoPath, yamlPath string, sites []oracleCallSite) error {
	var sb strings.Builder
	sb.WriteString("# migrate-oracle: calls requiring human review\n\n")
	sb.WriteString("Generated by `kitsoki migrate-oracle`. Each entry below is a call that\n")
	...
	for i, s := range sites {
		...
		sb.WriteString("  - Change to `invoke: host.oracle.decide` and add a `schema:` field.\n")
		sb.WriteString("  - Change to `invoke: host.oracle.ask` if the output is prose-only.\n")
		sb.WriteString("  - Change to `invoke: host.oracle.task` if the agent mutates files.\n")
		sb.WriteString("\n")
	}
...
// oracleMapKeyString converts an AST map key to a string.
func oracleMapKeyString(k ast.MapKeyNode) string {
	if k == nil {
		return ""
	}
