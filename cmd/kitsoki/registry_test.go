...
func TestRegistry_DaemonRestoresStableJobAndState(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	storiesDir, appPath := writeStory(t, "mini", []byte(minimalStory))
	dbPath := filepath.Join(t.TempDir(), "sessions.db")
	base := runtimeBase{DBPath: dbPath, ExecMode: orchestrator.ExecStaged, Flow: &testrunner.FlowFixture{}}

	reg1 := NewRegistry(webconfig.WebConfig{}, []string{storiesDir}, base)
	require.NoError(t, reg1.EnableDaemon(dbPath))
	_, err := reg1.Rescan()
	require.NoError(t, err)
	id, err := reg1.NewSession(context.Background(), appPath)
	require.NoError(t, err)
	entry, ok := reg1.Get(id)
	require.True(t, ok)
	_, err = entry.Driver.SubmitDirect(context.Background(), "go", nil)
	require.NoError(t, err)
	jobs, err := reg1.ListArtifactJobs(context.Background())
	require.NoError(t, err)
	require.Len(t, jobs, 1)
	assert.Equal(t, id, jobs[0].JobID)
	assert.Equal(t, "/s/"+id, jobs[0].RunURL)
	assert.Equal(t, "hall", jobs[0].Phase)
	reg1.Close()

	reg2 := NewRegistry(webconfig.WebConfig{}, []string{storiesDir}, base)
	t.Cleanup(reg2.Close)
	require.NoError(t, reg2.EnableDaemon(dbPath))
	_, err = reg2.Rescan()
	require.NoError(t, err)
	restored, err := reg2.RestoreDaemonJobs(context.Background())
	require.NoError(t, err)
	assert.Equal(t, 1, restored)
	restoredEntry, ok := reg2.Get(id)
	require.True(t, ok, "the original job id must remain the routable id")
	snap, err := restoredEntry.Source.Snapshot()
	require.NoError(t, err)
	assert.Equal(t, "hall", snap.Session.CurrentState)
	restoredJobs, err := reg2.ListArtifactJobs(context.Background())
	require.NoError(t, err)
	require.Len(t, restoredJobs, 1)
	assert.Equal(t, "running", restoredJobs[0].Status)
	assert.Empty(t, restoredJobs[0].InterruptedReason)
}

func TestRegistry_DaemonKeepsTerminalJobHistoryWithoutRestoringIt(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	storiesDir, appPath := writeStory(t, "quit", []byte(terminalQuitStory))
	dbPath := filepath.Join(t.TempDir(), "sessions.db")
	base := runtimeBase{DBPath: dbPath, ExecMode: orchestrator.ExecStaged, Flow: &testrunner.FlowFixture{}}
	...
	entry, ok := reg1.Get(id)
	require.True(t, ok)
	_, err = entry.Driver.SubmitDirect(context.Background(), "quit", nil)
	require.NoError(t, err)
	jobs, err := reg1.ListArtifactJobs(context.Background())
	require.NoError(t, err)
	require.Len(t, jobs, 1)
	...
	restored, err := reg2.RestoreDaemonJobs(context.Background())
	require.NoError(t, err)
	assert.Zero(t, restored, "terminal jobs remain history and are not reattached")
	_, live := reg2.Get(id)
	assert.False(t, live)
	jobs, err = reg2.ListArtifactJobs(context.Background())
	require.NoError(t, err)
	require.Len(t, jobs, 1)
	assert.Equal(t, "done", jobs[0].Status)
	assert.Equal(t, "/s/"+id, jobs[0].RunURL)
