...
func (r *SessionRegistry) NewSession(ctx context.Context, storyPath string) (string, error) {
	...
	e := &entry{
		...
		source:        live,
		driver:        server.OrchestratorDriver{Orch: orch, SID: sid, Jobs: rt.JobStore, Chats: rt.ChatStore},
		sink:          sink,
	}
...
func (r *SessionRegistry) AttachExternal(ctx context.Context, storyPath, key string) (string, error) {
	...
	driver := server.NewLockingDriver(server.OrchestratorDriver{Orch: orch, SID: sid, Jobs: rt.JobStore, Chats: rt.ChatStore}, lock)
