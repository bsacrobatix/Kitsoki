// migrate_oracle_roundtrip_test.go — round-trip parse assertions for the
// migrate-oracle codemod. Every rewritten YAML must parse without error
...
// mustRewriteOracleYAML is a test helper that writes src to a temp file,
// runs the migrate-oracle codemod against it, reads back the result, and
// returns both the rewritten bytes and the temp path.
func mustRewriteOracleYAML(t *testing.T, src []byte) (rewritten []byte, path string) {
	t.Helper()
	dir := t.TempDir()
	...
	cmd.SetOut(&stdout)
	cmd.SetArgs([]string{"migrate-oracle", path})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("migrate-oracle execute: %v", err)
	}
...
// TestMigrateOracle_RoundTrip_UnquotedVerb is the regression test for the
...
func TestMigrateOracle_RoundTrip_UnquotedVerb(t *testing.T) {
	t.Parallel()
	const src = `app:
  id: test
states:
  s:
    on_enter:
      - invoke: host.oracle.ask_with_mcp
        with: {prompt: x.md}
`
	rewritten, _ := mustRewriteOracleYAML(t, []byte(src))

	// Must contain the new verb, not the old one.
	if !strings.Contains(string(rewritten), "host.oracle.ask") {
		t.Fatalf("expected host.oracle.ask in rewritten YAML:\n%s", rewritten)
	}
	if strings.Contains(string(rewritten), "host.oracle.ask_with_mcp") {
...
// TestMigrateOracle_RoundTrip_SingleQuotedVerb covers single-quoted verbs.
func TestMigrateOracle_RoundTrip_SingleQuotedVerb(t *testing.T) {
	t.Parallel()
	const src = `app:
  id: test
states:
...
// TestMigrateOracle_RoundTrip_DoubleQuotedVerb covers double-quoted verbs.
func TestMigrateOracle_RoundTrip_DoubleQuotedVerb(t *testing.T) {
	t.Parallel()
	const src = `app:
  id: test
states:
...
// TestMigrateOracle_RoundTrip_TrailingWhitespace covers verbs followed by
// trailing whitespace before the newline (e.g. editor artefacts).
func TestMigrateOracle_RoundTrip_TrailingWhitespace(t *testing.T) {
	t.Parallel()
	// Trailing space after the verb value — YAML ignores it; the token value
	// is still the verb string without the space.
	src := []byte("app:\n  id: test\nstates:\n  s:\n    on_enter:\n      - invoke: host.oracle.ask_with_mcp   \n        with: {prompt: x.md}\n")
	rewritten, _ := mustRewriteOracleYAML(t, src)
	assertParseable(t, rewritten)
}

// TestMigrateOracle_RoundTrip_WithComment covers a verb on a line that has
// a neighbouring comment in the mapping.
func TestMigrateOracle_RoundTrip_WithComment(t *testing.T) {
	t.Parallel()
	const src = `app:
  id: test
states:
...
// TestMigrateOracle_RoundTrip_MultipleVerbs covers a file with more than one
...
func TestMigrateOracle_RoundTrip_MultipleVerbs(t *testing.T) {
	t.Parallel()
	const src = `app:
  id: test
states:
...
`
	...
	if strings.Contains(string(rewritten), "host.oracle.talk") {
		t.Fatalf("talk verb must be replaced:\n%s", rewritten)
	}
	// Expect: a→ask, b→decide, c→converse
	if !strings.Contains(string(rewritten), "host.oracle.ask") {
		t.Fatalf("expected host.oracle.ask for state a:\n%s", rewritten)
	}
	if !strings.Contains(string(rewritten), "host.oracle.decide") {
		t.Fatalf("expected host.oracle.decide for state b:\n%s", rewritten)
	}
	if !strings.Contains(string(rewritten), "host.oracle.converse") {
		t.Fatalf("expected host.oracle.converse for state c:\n%s", rewritten)
	}
}

// ── round-trip assertions backfilled into every TestMigrateOracleFile_* ──

// TestMigrateOracleFile_TalkToConverse_RoundTrip asserts yamlWithTalk parses
// after rewrite.
func TestMigrateOracleFile_TalkToConverse_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithTalk))
	assertParseable(t, rewritten)
}

// TestMigrateOracleFile_SchemaToDecide_RoundTrip asserts yamlWithSchema parses
// after rewrite.
func TestMigrateOracleFile_SchemaToDecide_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithSchema))
	assertParseable(t, rewritten)
}

// TestMigrateOracleFile_NoSchemaToAsk_RoundTrip asserts yamlWithNoSchema parses
// after rewrite.
func TestMigrateOracleFile_NoSchemaToAsk_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithNoSchema))
	assertParseable(t, rewritten)
}

// TestMigrateOracleFile_ChatIDToConverse_RoundTrip asserts yamlWithChatID
// parses after rewrite.
func TestMigrateOracleFile_ChatIDToConverse_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithChatID))
	assertParseable(t, rewritten)
}

// TestMigrateOracleFile_MutationToTask_RoundTrip asserts yamlWithMutation
// parses after rewrite.
func TestMigrateOracleFile_MutationToTask_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithMutation))
	assertParseable(t, rewritten)
}

// TestMigrateOracleFile_MutationFull_RoundTrip asserts yamlWithMutationFull
// parses after the with: block restructure.
func TestMigrateOracleFile_MutationFull_RoundTrip(t *testing.T) {
	t.Parallel()
	rewritten, _ := mustRewriteOracleYAML(t, []byte(yamlWithMutationFull))
	assertParseable(t, rewritten)
}

// TestMigrateOracle_BugfixRoomShape_Implementing verifies that the codemod,
...
func TestMigrateOracle_BugfixRoomShape_Implementing(t *testing.T) {
	t.Parallel()
  ...
  implementing:
    on_enter:
      - when: "world.workdir != ''"
        ...
        on_error: idle
`
	rewritten, _ := mustRewriteOracleYAML(t, []byte(preMigration))
	assertParseable(t, rewritten)
	s := string(rewritten)

	if !strings.Contains(s, "invoke: host.oracle.task") {
		t.Fatalf("expected host.oracle.task:\n%s", s)
	}
