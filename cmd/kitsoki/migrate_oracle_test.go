// migrate_oracle_test.go — tests for `kitsoki migrate-oracle` codemod (Phase 6).
//
// Tests cover each of the six classification cases from §6 of the oracle-split
...
func TestClassify_TalkVerb_Converse(t *testing.T) {
	t.Parallel()
	s := oracleCallSite{OriginalVerb: "host.oracle.talk"}
	classifyOracleCallSite(&s)
	if s.Classification != classConverse {
		t.Fatalf("host.oracle.talk should classify as converse, got %s", s.Classification)
	}
}

func TestClassify_ChatID_Converse(t *testing.T) {
	t.Parallel()
	s := oracleCallSite{OriginalVerb: "host.oracle.ask_with_mcp", HasChatID: true}
	classifyOracleCallSite(&s)
	if s.Classification != classConverse {
...
func TestClassify_MutationTools_Task(t *testing.T) {
	t.Parallel()
	s := oracleCallSite{OriginalVerb: "host.oracle.ask_with_mcp", HasMutationTool: true}
	classifyOracleCallSite(&s)
...
func TestClassify_WorkerValidator_Task(t *testing.T) {
	t.Parallel()
	s := oracleCallSite{
		OriginalVerb:      "host.oracle.ask_with_mcp",
...
func TestClassify_NoSchemaNomination_Ask(t *testing.T) {
	t.Parallel()
	s := oracleCallSite{OriginalVerb: "host.oracle.ask_with_mcp"}
	classifyOracleCallSite(&s)
...
func TestNewOracleVerb_AllCases(t *testing.T) {
	t.Parallel()
	cases := []struct {
		cls  oracleCallClassification
		want string
	}{
		{classExtract, "host.oracle.extract"},
		{classDecide, "host.oracle.decide"},
		{classAsk, "host.oracle.ask"},
		{classTask, "host.oracle.task"},
		{classConverse, "host.oracle.converse"},
		{classAmbiguous, ""},
	}
	for _, tc := range cases {
		got := newOracleVerb("host.oracle.ask_with_mcp", tc.cls)
		if got != tc.want {
...
func TestFindOracleCallSites_Talk(t *testing.T) {
	t.Parallel()
	sites, err := findOracleCallSites("test.yaml", []byte(yamlWithTalk))
	if err != nil {
	...
	if sites[0].OriginalVerb != "host.oracle.talk" {
		t.Fatalf("expected talk verb, got %q", sites[0].OriginalVerb)
	}
}

func TestFindOracleCallSites_AskWithMCP(t *testing.T) {
	t.Parallel()
	sites, err := findOracleCallSites("test.yaml", []byte(yamlWithSchema))
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
	}
	if len(sites) != 1 {
	...
	if sites[0].OriginalVerb != "host.oracle.ask_with_mcp" {
		t.Fatalf("expected ask_with_mcp verb, got %q", sites[0].OriginalVerb)
	}
}

func TestFindOracleCallSites_DetectsSchema(t *testing.T) {
	t.Parallel()
	sites, err := findOracleCallSites("test.yaml", []byte(yamlWithSchema))
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
...
func TestFindOracleCallSites_DetectsChatID(t *testing.T) {
	t.Parallel()
	sites, err := findOracleCallSites("test.yaml", []byte(yamlWithChatID))
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
	}
	if len(sites) == 0 {
...
func TestFindOracleCallSites_DetectsMutationTools(t *testing.T) {
	t.Parallel()
	sites, err := findOracleCallSites("test.yaml", []byte(yamlWithMutation))
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
	}
	if len(sites) == 0 {
...
func TestMigrateOracleFile_TalkToConverse(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	...
	if err := cmd.Execute(); err != nil {
		t.Fatalf("migrate-oracle: %v", err)
	}
	got, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	if !strings.Contains(string(got), "host.oracle.converse") {
		t.Fatalf("expected host.oracle.converse in rewritten YAML, got:\n%s", got)
	}
	if strings.Contains(string(got), "host.oracle.talk") {
		t.Fatalf("host.oracle.talk should have been replaced:\n%s", got)
	}
}

func TestMigrateOracleFile_SchemaToDecide(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
	...
	if !strings.Contains(string(got), "host.oracle.decide") {
		t.Fatalf("expected host.oracle.decide in rewritten YAML, got:\n%s", got)
	}
}

func TestMigrateOracleFile_NoSchemaToAsk(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
	...
	if !strings.Contains(string(got), "host.oracle.ask") {
		t.Fatalf("expected host.oracle.ask in rewritten YAML, got:\n%s", got)
	}
}

func TestMigrateOracleFile_DryRun_NoWrite(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
	...
	var stdout strings.Builder
	cmd.SetOut(&stdout)
	cmd.SetArgs([]string{"migrate-oracle", "--dry-run", path})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("migrate-oracle --dry-run: %v", err)
	}
	got, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read: %v", err)
...
func TestMigrateOracleFile_AmbiguousTODO(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
...
func TestMigrateOracleFile_PrintClassified(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
	if err := os.WriteFile(path, []byte(yamlWithSchema), 0644); err != nil {
	...
	var stdout strings.Builder
	cmd.SetOut(&stdout)
	cmd.SetArgs([]string{"migrate-oracle", "--print-classified", path})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("migrate-oracle --print-classified: %v", err)
	}
...
func TestMigrateOracleFile_MutationToTask_RestructuresWithBlock(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "app.yaml")
	...
	if !strings.Contains(s, "host.oracle.task") {
		t.Fatalf("expected host.oracle.task, got:\n%s", s)
	}
	if strings.Contains(s, "host.oracle.ask_with_mcp") {
...
// TestFindOracleCallSites_FlowStyleTools verifies that flow-style tools:
// [Edit, Write] is detected via AST traversal (M10 false-positive fix).
func TestFindOracleCallSites_FlowStyleTools(t *testing.T) {
	t.Parallel()
	const yaml = `
app:
  id: test-app
    ...
    on_enter:
      - invoke: host.oracle.ask_with_mcp
        with:
          ...
          schema: schemas/result.json
`
	sites, err := findOracleCallSites("test.yaml", []byte(yaml))
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
	}
	if len(sites) == 0 {
...
// TestFindOracleCallSites_NoFalsePositive_CommentAndBind verifies that
// "Edit" appearing in a comment or bind: does NOT set HasMutationTool (M10).
func TestFindOracleCallSites_NoFalsePositive_CommentAndBind(t *testing.T) {
	t.Parallel()
	const yaml = `
app:
  id: test-app
