...
authentication.`,
	...
	cmd.Flags().StringVar(&ticketRepo, "ticket-repo", "constructorfabric/Kitsoki", "file Report-bug reports as GitHub issues on this owner/repo (evidence saved under .artifacts/bug-reports for developer review) instead of a local issues/bugs/*.md file; requires GitHub auth from `kitsoki gh-agent token` or GH_TOKEN/GITHUB_TOKEN. Pass an empty string to write local issues/bugs/*.md files instead")
