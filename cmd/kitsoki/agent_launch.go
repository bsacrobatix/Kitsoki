...
// The freestanding-agent TOML loader lives in internal/agentroot (the
// agent-mode foundation shares it); the alias type and thin delegations below
// keep every `agent launch` call site and its tests unchanged.
type standaloneCodexAgent = agentroot.StandaloneCodexAgent
...
var materializeBuiltInAgentLibrary agentroot.LibraryMaterializer = baseskills.Materialize
...
func resolveStandaloneAgentFile(agentName, explicit, template string) (string, func(), error) {
	return agentroot.ResolveStandaloneAgentFile(agentName, explicit, template, materializeBuiltInAgentLibrary)
}

func loadStandaloneCodexAgent(path string) (standaloneCodexAgent, error) {
	return agentroot.LoadStandaloneCodexAgent(path)
}
...
func launchTOMLString(s string) string { return agentroot.TOMLString(s) }
