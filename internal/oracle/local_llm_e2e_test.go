...
func TestLocalLLMManagedEndToEnd(t *testing.T) {
	...
	orc := oracle.NewLocalLLM(server.DefaultModel, freePort(t), "", true /*grammar*/, "" /*managed*/, nil)
	defer orc.Close()
	...
	if verr := oracle.ValidateSubmission(json.RawMessage(schema), resp.Submission); verr != nil {
	...
	if verr := oracle.ValidateSubmission(json.RawMessage(schema), resp2.Submission); verr != nil {
