// local_llm_sidecar.go provides the seam between LocalLLMOracle and the managed
// sidecar lifecycle (internal/oracle/server).
//
// Why a seam here: LocalLLMOracle (local_llm.go) is independently useful and
...
// interface the Oracle depends on; both modes (endpoint and managed) are handled
// by the server package, so this file is a thin constructor wrapper.

package oracle

import "kitsoki/internal/oracle/server"

// newSidecar constructs the localSidecar backing a LocalLLMOracle. It returns a
