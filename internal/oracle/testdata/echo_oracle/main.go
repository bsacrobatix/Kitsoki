// echo_oracle is a minimal JSON-RPC 2.0 oracle subprocess for testing.
//
// It reads newline-delimited JSON-RPC 2.0 requests on stdin and writes
// responses on stdout. The oracle.ask method is supported; the response
...
func main() {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &req); err != nil {
			fmt.Fprintf(os.Stderr, "echo_oracle: unmarshal request: %v\n", err)
			os.Exit(1)
		}
