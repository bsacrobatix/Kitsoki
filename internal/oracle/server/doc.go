...
// llama.cpp OpenAI-compatible HTTP server that [kitsoki/internal/oracle].LocalLLMOracle
...
// health-gate, and orderly teardown — kept out of the oracle transport so the
// transport stays a pure HTTP client and stays trivially testable in endpoint
// mode.
//
// The oracle transport depends only on the narrow [Sidecar.EnsureRunning] /
...
//     mirroring the os/exec surface SubprocessOracle uses. [NewSpawner] returns
...
//     until GET /health returns 200, so the first oracle POST never races a
//     half-open server.
//   - Close is idempotent and ordered: SIGTERM first, escalate to SIGKILL only
//     after a grace window (terminateTimeout, mirroring oracle's
...
//     /v1/chat/completions call lives in [kitsoki/internal/oracle].LocalLLMOracle.
...
//     sidecar's (mirrors the oracle package's no-retry stance).
package server
