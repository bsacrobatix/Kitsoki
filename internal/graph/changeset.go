...
type Operation struct {
	...
	// Node is the target node id for added/modified/removed. For renamed,
	// use From/To instead (NodeID is catalog-global, so a rename is a
	// distinct shape, not a field edit).
	Node NodeID

	After   map[string]any // added: the full new node mapping (must include "id" == Node)
	Before  map[string]any // removed: the full expected current node mapping (audit + stale-guard)
	Changes []FieldChange  // modified
