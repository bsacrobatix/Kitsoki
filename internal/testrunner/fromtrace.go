...
//
// Why a cassette rather than host_handlers: a session's host/oracle responses
// vary per call (e.g. host.oracle.converse returns a different reply each of the
...
// episodeIDSlug turns a handler namespace ("host.oracle.converse") into a
// readable, YAML-safe episode-id slug ("host_oracle_converse").
...
const flowHeader = "# Generated by `kitsoki trace to-flow`.\n" +
	...
	"# would hard-fail on the first drift. Host/oracle responses replay from the\n" +
