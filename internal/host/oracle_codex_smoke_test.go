...
// codexBackend.ValidatorToolName. Gated on KITSOKI_ORACLE_LIVE=1 (a real Codex
...
func TestCodexLiveSmoke(t *testing.T) {
	if os.Getenv("KITSOKI_ORACLE_LIVE") != "1" {
		t.Skip("set KITSOKI_ORACLE_LIVE=1 to run the live codex smoke test (real Codex request)")
	}
	...
	cmd := exec.CommandContext(ctx, codexBin, "exec",
		...
		// translator's base args (see oracle_backend_codex.go).
