...
// TestOracleSearchRealDocs runs the handler against the actual docs/ tree in
...
func TestOracleSearchRealDocs(t *testing.T) {
	...
	store := embed.NewStore(storeDir)
	handler := NewOracleSearchHandler("nomic-embed-text-v1.5", repoRoot, fakeEmbedder, store)
