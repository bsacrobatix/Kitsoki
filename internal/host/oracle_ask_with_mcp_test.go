...
func TestOracleAskWithMCP_ExplicitArgsScopesPromptVariables(t *testing.T) {
	...
	assert.Contains(t, out, "ticket=PROJ-12345")
...
func TestOracleAskWithMCP_ServersMaterialized(t *testing.T) {
	...
	mcpServers := map[string]any{
		"wiggum": map[string]any{
			"command": "python3",
			"args":    []any{"tools/validator/wiggum-mcp.py", "--schema", "schemas/03.json"},
		},
	}
...
func TestOracleAskWithMCP_ValidatorBlockParsed(t *testing.T) {
	...
	//   --post-cmd-arg ticket=PROJ-89912
	...
	require.Contains(t, got, "ticket=PROJ-89912")
	...
	tIdx := indexOfString(got, "ticket=PROJ-89912")
...
// for the bug observed in PROJ-89912 where a YAML `max_retries: 5` arrived
...
func TestOracleAskWithMCP_ValidatorMaxRetriesIntegerWidths(t *testing.T) {
	...
	cases := []struct {
		...
		{"uint16", uint16(5)},
		{"uint32", uint32(5)},
		{"uint64", uint64(5)}, // PROJ-89912 regression: goccy/go-yaml produces this
		{"float32", float32(5)},
		{"float64", float64(5)},
	}
...
func TestOracleAskWithMCP_ValidatorBlockArgsTemplated(t *testing.T) {
	...
	res, err := host.OracleAskWithMCPHandler(context.Background(), map[string]any{
		...
		"validator": map[string]any{
			...
			"post_cmd_args": map[string]any{
				...
				"ticket":   "PROJ-12345",
				"worktree": "/tmp/PROJ-12345-3/worktree",
			},
		},
		"output_format": "json",
	})
	...
	assert.Contains(t, got, "ticket=PROJ-12345")
	assert.Contains(t, got, "worktree=/tmp/PROJ-12345-3/worktree")
}
