...
// oracle_ask_test.go — Phase 3 tests for host.oracle.ask.
//
// Coverage:
...
//   - Streaming: tokens stream through OracleStreamer (stub path).
...
func TestOracleAsk_RequiresPromptPath(t *testing.T) {
	t.Parallel()
...
func TestOracleAsk_PromptAliasAccepted(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	...
	res, err := host.OracleAskHandler(ctx, map[string]any{"prompt": p})
	if err != nil {
		t.Fatalf("unexpected Go error: %v", err)
	}
	if res.Error != "" {
...
func TestOracleAsk_AgentOptional(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
func TestOracleAsk_RejectsMutationTool_Edit(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
func TestOracleAsk_RejectsMutationTool_Write(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
func TestOracleAsk_PerCallTools_RejectsMutation(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
func TestOracleAsk_BashWithoutProfile_Rejected(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
func TestOracleAsk_BashWithReadOnlyProfile_Allowed(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
	if err := os.WriteFile(p, []byte("hello"), 0o644); err != nil {
...
// These tests exercise ApplyBashProfile directly (already in oracle_phase1_test.go
...
func TestOracleAsk_TokensFlowThroughOracleStreamer(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
...
func TestOracleAsk_SchemaMode_SubmittedPresentWhenSchemaSet(t *testing.T) {
	t.Parallel()
...
func TestOracleAsk_SchemaMode_SubmittedFromTempFile(t *testing.T) {
	t.Parallel()
	...
	// the existing oracle_ask_with_mcp_test.go integration tests.
...
// TestOracleAsk_SchemaMode_SubmittedFromTempFile to simulate the mcp-validator
...
// TestOracleAsk_SchemaModeSubmitFileCleaned verifies that after the handler
// returns the submit tempfile no longer exists (M7: deferred cleanup).
func TestOracleAsk_SchemaModeSubmitFileCleaned(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	promptFile := filepath.Join(dir, "p.md")
	if err := os.WriteFile(promptFile, []byte("explain"), 0o644); err != nil {
...
// TestOracleAsk_InlinePrompt_AcceptsMultiLineContent verifies that when
// prompt: contains multi-line content (e.g. from stdin), the handler uses it
// as inline text without trying to open it as a file.
func TestOracleAsk_InlinePrompt_AcceptsMultiLineContent(t *testing.T) {
	t.Parallel()
...
// TestOracleAsk_InlinePrompt_FilePath_StillWorks verifies that when prompt:
...
func TestOracleAsk_InlinePrompt_FilePath_StillWorks(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	p := filepath.Join(dir, "p.md")
...
// ── L11: OracleStreamer.CLIArgs positional-arg check ──────────────────────

// TestOracleStreamer_CLIArgs_PanicsOnPositionalArgInTests verifies that
// OracleStreamer.Run panics in tests when CLIArgs contains a positional
// argument (not a flag value — i.e. not immediately following a flag name).
func TestOracleStreamer_CLIArgs_PanicsOnPositionalArgInTests(t *testing.T) {
	t.Parallel()
	defer func() {
		if r := recover(); r == nil {
