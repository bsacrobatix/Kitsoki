...
// Hermetic oracle isolation: every claude-CLI invocation a story makes must
...
func TestOracleSettingSources_Ask(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestOracleSettingSources_ConverseNoChat(t *testing.T) {
	t.Parallel()
	var captured []string
	ctx := host.WithClaudeRunner(context.Background(), capturingRunner(&captured))

	if _, err := host.OracleConverseHandler(ctx, map[string]any{
		"question": "ping",
	}); err != nil {
		t.Fatalf("unexpected Go error: %v", err)
	}
...
func TestOracleSettingSources_ConverseChat(t *testing.T) {
	t.Parallel()
	cs := newFakeChatStore()
