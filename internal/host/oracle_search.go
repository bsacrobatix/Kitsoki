// Package host — host.oracle.search handler.
//
// host.oracle.search embeds a local corpus (selected by glob patterns) and
...
//
// Use NewOracleSearchHandler to obtain a handler that has a real Embedder and
// Store wired in. The bare OracleSearchHandler sentinel is registered by
...
// OracleSearchHandler is a no-op sentinel so the verb "host.oracle.search" is
...
// NewOracleSearchHandler before use.
func OracleSearchHandler(ctx context.Context, args map[string]any) (Result, error) {
	return Result{Error: "host.oracle.search: no embedder configured; wire via NewOracleSearchHandler"}, nil
}

// NewOracleSearchHandler returns a host.oracle.search handler closed over the
...
func NewOracleSearchHandler(model string, workingDir string, embedder embed.Embedder, store *embed.Store) func(context.Context, map[string]any) (Result, error) {
	return func(ctx context.Context, args map[string]any) (Result, error) {
		...
		if query == "" {
			return Result{Error: "host.oracle.search: query argument is required"}, nil
		}

		// --- corpus ---
		corpusGlobs, err := extractCorpusGlobs(args)
		if err != nil {
			return Result{Error: fmt.Sprintf("host.oracle.search: %v", err)}, nil
		}
		if len(corpusGlobs) == 0 {
			return Result{Error: "host.oracle.search: corpus argument is required"}, nil
		}
		...
		if err != nil {
			return Result{Error: fmt.Sprintf("host.oracle.search: ingest: %v", err)}, nil
		}
		...
		if store != nil {
			...
			if loadErr != nil {
				return Result{Error: fmt.Sprintf("host.oracle.search: store load: %v", loadErr)}, nil
			}
			if hit {
				entries = cached
			}
		...
		if entries == nil {
			...
			if err != nil {
				return Result{Error: fmt.Sprintf("host.oracle.search: embed corpus: %v", err)}, nil
			}
			if store != nil {
		...
		if err != nil {
			return Result{Error: fmt.Sprintf("host.oracle.search: embed query: %v", err)}, nil
		}
		if len(queryVecs) == 0 {
			return Result{Error: "host.oracle.search: embedder returned no vector for query"}, nil
		}
