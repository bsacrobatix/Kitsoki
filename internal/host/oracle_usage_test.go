...
// the OracleReturned event's Meta, via the per-call usage box installed by the
// orchestrator's host dispatch (host.WithOracleUsageBox).
...
// TestOracleAsk_UsageMeta asserts the result event's token usage + cost reach
// OracleReturned.Meta when a usage box is installed (the production wiring).
func TestOracleAsk_UsageMeta(t *testing.T) {
	t.Parallel()
	...
	ctx := host.WithOracleUsageBox(oracleCtxForTest(sink))
	...
	returned := findEvent(t, sink.events, store.OracleReturned)
	var payload struct {
	...
	if payload.Meta == nil {
		t.Fatal("OracleReturned.Meta is nil — token usage was not captured")
	}
...
// TestOracleAsk_UsageMeta_NoBox asserts the call still succeeds (Meta omitted)
// when no usage box is installed — degrading gracefully for paths that haven't
// wired one.
func TestOracleAsk_UsageMeta_NoBox(t *testing.T) {
	t.Parallel()
	...
	ctx := host.WithClaudeRunner(oracleCtxForTest(sink), fakeStreamRunner("final answer"))
	res, err := host.OracleAskHandler(ctx, map[string]any{"prompt_path": promptPath})
	if err != nil {
		t.Fatalf("OracleAskHandler: %v", err)
	}
