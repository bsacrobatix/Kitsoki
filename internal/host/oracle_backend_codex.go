// Package host — the codex oracleBackend.
//
// codexBackend drives OpenAI's `codex exec` CLI as a third drop-in alternative
// to `claude` and `copilot` for every oracle verb. As with copilot, the verb
...
//     why `--oracle codex` requires a trusted/externally-sandboxed environment;
...
const CodexBinEnv = "KITSOKI_ORACLE_CODEX_BIN"
...
func (codexBackend) TranslateInvocation(claudeArgs []string, stdin, workingDir string) Invocation {
	...
	args = append(args,
		...
		// operator opts in by selecting `--oracle codex`.
...
// gated live smoke test (oracle_codex_smoke_test.go, KITSOKI_ORACLE_LIVE=1) and
// Live-verified (KITSOKI_ORACLE_LIVE=1 smoke, 2026-06-11): codex names the MCP
