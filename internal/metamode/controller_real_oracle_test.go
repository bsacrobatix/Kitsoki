...
// OracleCaller adapter. The fake `claude` binary
// (internal/host/testdata/fake-oneshot-mcp.sh) is invoked as a real
// subprocess via host.OracleBinEnv — this proves the controller +
...
// internal/host/oracle_ask_with_mcp_test.go.
...
// TestController_Send_RealOracleAdapter_RoundTrip is the end-to-end
// wire test: real metamode.NewOracleCallerAdapter() in front of a fake
// `claude` subprocess launched via host.OracleBinEnv. The fake echoes
// its stdin so we get a non-empty reply, and the host mints a fresh
// claude session id since no prior one is set on the chat.
func TestController_Send_RealOracleAdapter_RoundTrip(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("fake-oneshot-mcp.sh requires bash")
	}

	// Point the real oracle adapter at the fake claude binary.
	t.Setenv(host.OracleBinEnv, fakeBinPath(t))
