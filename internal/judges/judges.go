...
// the "is this verdict auto-fireable?" rule lives, so the contract
// gate clause in stories/bugfix/rooms/*.yaml, the story flow tests,
// and any future tooling all agree on the same semantics.
//
...
// schemaJSON mirrors the verdict schema from the dev-story
// implementation contract.  Keep this in lockstep with
// stories/bugfix/schemas/judge_verdict.json — that JSON file is the
// source of truth; if it ever drifts, update this constant.
const schemaJSON = `{
