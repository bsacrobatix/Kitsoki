...
grep -q 'EnvironmentFile=-/etc/kitsoki/pog-runtime-db.env' "$assets/pog-portal.service"
! grep -q '^Environment=POG_AGENT_RUNNER_DB=' "$assets/pog-portal.service"
...
grep -q 'Postgres backend verification did not pass' "$assets/install.sh"
grep -q 'db verify' "$assets/install.sh"
grep -q 'postgres-verification.json' "$assets/install.sh"
grep -q 'runtime_db_env=/etc/kitsoki/pog-runtime-db.env' "$assets/install.sh"
grep -q 'POG portal did not receive the Postgres backend' "$assets/install.sh"
grep -q 'POG portal retained a forbidden SQLite runner ledger override' "$assets/install.sh"
grep -q 'colony runner retained a forbidden SQLite runner ledger override' "$assets/install.sh"
...
grep -q 'db verify' "$deploy"
grep -q 'postgres-verification.json' "$deploy"
grep -q 'POG_AGENT_RUNNER_DB' "$deploy"
grep -q 'portal_process_environment' "$deploy"
...
grep -q 'printf .POG_AGENT_RUNNER_DB=/var/lib/kitsoki-pog/sessions.db' "$assets/install.sh"
...
grep -Fq -- '--gate-tier full' "$assets/kitsoki-queue-worker-hosted-engine.conf"
grep -Fq -- '--executor-pipeline full' "$assets/kitsoki-queue-worker-hosted-engine.conf"
...
grep -Fq -- '--gate-tier change' "$assets/kitsoki-pog-integration-current-worker.service"
