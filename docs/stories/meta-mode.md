...
| `/work [--all]`          | on-path or meta      | Prints active async work. Without flags it is scoped to the current Kitsoki session: unread notifications, running/awaiting/failed jobs, queued chat drives, and background Claude chats. `--all` also includes background Claude chats across every Kitsoki session on this host and seeds the same numbering cache as `/sessions list`, so `/sessions attach <N>` can switch directly to one. |
...
- **`/attach` hands the terminal to claude.** While `/meta`
  ...
  `/work --all` lists background claude sessions alongside the
