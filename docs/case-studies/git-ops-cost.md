...
```bash
make cost-report                       # all stories -> .artifacts/cost-report/cost-report.{md,summary.json,slidey.json}
...
The Slidey output is generated by `tools/report-deck/deterministic_deck.py` from
the same structured cost summary. It is a standardized review deck with linked
artifacts and status tables, not LLM-authored prose.
