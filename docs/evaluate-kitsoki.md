...
| Capability | Kitsoki | [OpenClaw](https://openclaw.ai/) | [OpenCode](https://opencode.ai/) | [Devin](https://docs.devin.ai/get-started/devin-intro) autonomous agents | [GitHub Copilot cloud agent](https://docs.github.com/en/copilot/concepts/agents/cloud-agent/about-cloud-agent) |
|---|---|---|---|---|---|
| Primary job | Repeatable conversational workflows where the process must be explicit, testable, and replayable. | Personal/team assistant that acts through chat apps, integrations, memory, and a controlled machine. | Open-source coding agent for terminal, IDE, and desktop sessions. | Autonomous AI software engineer for tickets, bug fixes, feature work, and parallel agent fleets. | GitHub-native cloud agent for issue/branch/PR work in an ephemeral GitHub Actions environment. |
| Who owns the control flow? | The authored story graph: rooms, intents, guards, transitions, and effects. | The assistant/agent loop coordinates broad digital tasks. | The coding agent loop drives repo work inside a developer surface. | Devin owns the task plan and execution loop. | Copilot owns the delegated coding session inside GitHub's cloud agent flow. |
| Agent authority model | Agents are callees at declared points, with scoped tools and typed returns. | Broad assistant authority through configured skills and integrations. | Broad coding-agent authority inside the active project/session. | Broad autonomous-worker authority for software tasks. | Cloud-agent authority scoped by GitHub repo, branch, environment, and org settings. |
| Deterministic replay / no-LLM tests | First-class: flow fixtures and host cassettes replay whole conversations without live model spend. | Not the core product promise. | Not the core product promise. | Not the core product promise. | GitHub logs/commits make work reviewable, but replaying the whole conversational workflow is not the main abstraction. |
| Operator gates and handoff | Explicit rooms, choice intents, operator-ask, and traceable human decisions. | Human steers through chat and configured approvals. | Human steers the coding session. | Human delegates and reviews results. | Human assigns, reviews branch changes, and decides whether to open/merge PRs. |
| Best fit | Workflows that repeat, cross surfaces, need auditability, or must be defended after the run. | Always-on personal/team digital assistance. | Interactive coding with model/provider flexibility and local developer control. | Delegating software tasks to autonomous workers. | GitHub issue-to-branch-to-PR automation. |
| Kitsoki moat | One story definition drives runtime, web, TUI, MCP, traces, demos, and tests. | Kitsoki is narrower but more falsifiable: prove the workflow, not just the assistant outcome. | Kitsoki can wrap coding agents as bounded workers inside a larger deterministic process. | Kitsoki separates the process from the worker, so reproduce/propose/implement/test/review/validate can improve independently. | Kitsoki is not limited to GitHub's product boundary and can replay the same state machine across custom transports and operator surfaces. |
