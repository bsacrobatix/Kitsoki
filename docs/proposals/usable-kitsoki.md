...
| S2 | Never-silent runtime | runtime | Every `on_error:` traversal renders the error at least once before resting, gated by a G-FLOW invariant across all stories; near-miss routing goes to the workbench, never an adjacent command; one routing decision point instead of four call sites. | — | Draft | [`never-silent-runtime.md`](never-silent-runtime.md) |
