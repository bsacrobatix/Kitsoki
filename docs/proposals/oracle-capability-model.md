# Epic: a unified capability model for every oracle

**Status:** Draft v1. No slices implemented yet.
**Kind:**   epic
**Slices:** 3 (0/3 shipped) + a conformance check folded into `oracle-contract-eval.md`
...
`host.oracle.task` can write anywhere its tools reach (`working_dir` is not a
jail — `internal/host/oracle_task.go:284`), and the real incident behind
...
- `oracle.ask` / `oracle.decide` — a hardcoded `mutationTools` deny set
  ...
  `oracle_decide.go:42`); read-only by construction.
- `oracle.converse` — a `readOnlyDeniedTools` backstop
...
- `oracle.task` — fully unrestricted `bypassPermissions`
  (`oracle_task.go:191`); nothing reins it in.

And `external_side_effect` (`internal/app/types.go:999`) is one bit meaning
two things: the task **replay mode** (`inferReplayMode`,
`oracle_task_replay.go:85`) and the converse **read-only posture**
...
oracle through it, enforce it in layers, and audit that the boundary held.

## What changes

Once every slice has shipped, **every oracle — `decide`, `ask`, `converse`,
`task` — is governed by one capability model** built from four cooperating
layers, each feeding the next:

```
TOOLBOX        a named, reusable capability grant — the tools an oracle may use
...
- The **toolbox** is the single thing that restricts an oracle: a named set
...
- **Enforcement is derived from the effect class and applied uniformly** to
  all four oracle kinds: the tool-layer allowlist + mutator-deny for
...
The throughline: an author declares **one toolbox** per oracle; the engine
...
- **Spans:** runtime (3 slices) + tracing (the conformance check folded into
  the standalone `oracle-contract-eval.md`).
...
| 2 | toolbox + uniform enforcement | runtime | named `toolboxes:` + `tools_add:`; one effect-derived tool-layer policy for **all four** oracle kinds, replacing the three ad-hoc mechanisms | 1 | Draft | [`toolbox-and-enforcement.md`](toolbox-and-enforcement.md) |
...
| — | effect/toolbox conformance | tracing | offline check that a recorded call's tool uses never exceeded its toolbox/effect (a new Layer-1 sibling) | 1 | Draft | [`oracle-contract-eval.md`](oracle-contract-eval.md) (§Conformance) |
...
1. **Two axes, not one enum.** `effect` (class) and `deterministic` (bool)
   stay separate because a tool-less `oracle.extract` touches nothing (`pure`)
...
3. **The toolbox is the only grant surface.** An oracle's tools come from a
...
4. **Classification and enforcement are deterministic — the moat is
   ...
   recorded `oracle.decide` — a labeled datapoint, the moat applied to agent
   output.
...
2. **Does conformance belong in `oracle-contract-eval` or this epic?** Decided:
   folded into `oracle-contract-eval.md` as a new Layer-1 sibling (schema
...
3. **macOS / no-kernel-backend posture** — when no OS sandbox backend exists,
   ...
   tool-layer-only ever acceptable for a `write` oracle in CI? *Lean:
   warn-and-degrade by default, with a hard-fail knob (slice 3).* 

## Non-goals

- A general OS sandbox for all of kitsoki — only oracle subprocesses, and only
...
- The correctness-eval / backend-routing machinery itself — that stays in
  [`oracle-contract-eval.md`](oracle-contract-eval.md) and
  [`local-model-oracle.md`](local-model-oracle.md); this epic only adds the
  conformance sibling.
