# Runtime: `host.oracle.search` — semantic search over files on disk
...
A new host verb **`host.oracle.search`**. A room hands it a `query`, a
...
through the generative oracle registry (epic shared decision 2).
...
- **Code seams:**
  - new `internal/host/oracle_search.go` — `OracleSearchHandler(ctx, args)
    (Result, error)`, mirroring the shape of `oracle_decide.go:85` but
  ...
  - registered in the host-call registry alongside the other `oracle.*`
    handlers.
...
- **Backward compat:** purely additive. A story that never calls
  `oracle.search` is unchanged.
- **Docs on ship:** `docs/architecture/embeddings.md` (the `oracle.search`
  section) + a mention in [`docs/architecture/hosts.md`](../architecture/hosts.md)
  /`oracle-plugin.md` §2 (calling an oracle verb from a room).

## Vocabulary changes

| Kind | Name | Shape | Notes |
|---|---|---|---|
| host call | `host.oracle.search` | `{query, corpus, top_k?, min_score?, chunk?} → bind: []Hit` | reads files under `working_dir`; no mutation |
...
downstream `oracle.ask`/`oracle.decide` (or a view) consumes them. Keeping
search and generation separate keeps the retrieval decision independently
inspectable.

## Decision recording

`oracle.search` is an **interpretive decision** (which chunks are relevant) and
...
This rides the existing oracle-call breadcrumb shape so the trace and
runstatus surfaces render it as a recorded oracle decision, even though
dispatch bypasses the generative registry. Whether it reuses
`OracleCalled`/`OracleReturned` verbatim or gets a sibling `kind` is the open
question below.
...
- **Read-only.** The handler reads files under `working_dir` and writes
  ...
  call-time error. (`oracle.search` carries no write capability regardless of
  any toolbox grant — cf. [oracle-capability-model](oracle-capability-model.md).)
- **Sidecar:** reuses the embedding `Sidecar` from
  ...
  message (mirrors `local-model-oracle`'s `model:`-or-`endpoint:` invariant).
...
- [ ] 2.1 OracleSearchHandler: args validation, ingest→embed(Document)→store, embed(Query)→rank
...
- [ ] 4.1 Stateless: OracleSearchHandler with the fake Embedder ranks fixture files deterministically
- [ ] 4.2 Flow fixture: a room calls oracle.search over on-disk fixtures; result binds; legacy paths green
...
- [ ] 5.2 docs/architecture/embeddings.md oracle.search section; mark this slice shipped in the epic
```
...
1. **Breadcrumb kind.** Reuse `OracleCalled`/`OracleReturned` verbatim (the
   trace already renders them; `meta` carries the scores), or add a sibling
   `oracle.search` event kind so runstatus can badge retrieval distinctly?
...
- **A reranker / generation step.** `oracle.search` returns ranked text; an
  answer is a downstream `oracle.ask`/`oracle.decide`'s job (epic non-goal).
