...
**Status:** Draft v1. Nothing implemented yet. Slices are sketched below;
...
or export one typed operation through CLI, MCP, and JSON-RPC without
surface-specific glue.

That gap prevents a product from being implemented *as stories*. A product such
as POG should be able to keep workflow, application UI, and public operations in
...
- **TUI** projects cards, forms, and actions onto typed view elements and native
  terminal controls;
- **CLI, MCP, and JSON-RPC** expose the story's named typed operations through
  generated adapters backed by one operation registry.

Story imports, overrides, interfaces, and future kits compose application
fragments by the same ownership rules as workflow fragments. A base
`@kitsoki/wizard-ui` story can therefore provide a headless frame contract, the
default card presentation, or both; a product story can inherit it, replace
selected cards/components, and retain deterministic fallbacks for non-web
surfaces.

## Impact

- **Spans:** runtime, story, tui, tracing, web/VS Code tooling.
- **Net surface:** an application manifest and merge model; canonical frame and
  action envelopes; typed operation registry; Vue component loader; a
  Kitsoki-owned Vite dev/build lifecycle; adapters for web, VS Code, TUI, CLI,
...
| `host.starlark.run` (`docs/architecture/hosts.md`) | Deterministic implementation for typed application operations |
| `kitsoki turn`, `kitsoki serve`, Studio MCP, and runstatus JSON-RPC | Existing adapters to consolidate behind the operation registry |
...
Presentation renderers consume it; traces and replay record it.
...
- An action binds to an intent or exported operation. The runtime performs
  schema validation, stale-frame checks, authorization/effect checks,
  invocation, tracing, and frame regeneration.
...
Stories may export named operations without hand-writing one adapter per
transport. Stateful operations dispatch an intent against a session; functional
operations invoke a declared Starlark script or bound host-interface operation.
...
One registry owns discovery, validation, invocation, and receipts. Adapters are
mechanical:

| Adapter | Generated behavior |
|---|---|
| JSON-RPC | Discover and call the stable operation name with JSON input |
| MCP | Advertise a tool whose input schema and description come from the operation |
| CLI | `kitsoki app call <operation> --input <json-or-@file>` plus discovery/help |
| Web/VS Code/TUI | Dispatch the same operation from an action envelope |

The exact adapter spelling may differ where a protocol restricts names, but
discovery returns the canonical operation id. Every adapter produces the same
validated result/error envelope and a trace receipt keyed by operation id,
session, actor, effect class, input digest, output digest, and transport. No
adapter may embed business logic.
...
| VS Code native | Commands generated from actions; optional trees, quick picks, status items | Declarative native contributions bound to operations, never duplicate handlers |
...
| CLI | Operation discovery/call; structured JSON by default | Human-readable template for an operation result |
...
contract. `presentation: none` means “serve the frame and operations,” not
...
1. imported application pages, components, operations, and tokens remain
...
5. operation overrides must be input/output compatible and may not weaken
   effect or capability policy;
6. UI actions may target only exported intents/operations visible through the
   import boundary; and
...
The default wizard framework is itself an ordinary reusable story/application
fragment. It does not receive privileged business APIs. A presentation-free
consumer can import the same fragment for schemas, navigation, and actions
without loading Vue.
...
- every product page, card, form, navigation item, command, and product
  operation is declared by a POG story/application fragment;
- every business interaction resolves to a story intent or exported operation;
- custom POG Vue components live with and are loaded from those story packages;
- web and VS Code reuse the same presentation modules and operation contracts;
- CLI, MCP, and JSON-RPC discovery expose the same eligible operations;
...
- a generated ownership report has no unexplained product route, API handler,
  command, or UI entry outside the story manifests.

The acceptance proof must use a no-LLM POG fixture and scenario inventory. It
drives at least one end-to-end wizard workflow through web, VS Code reuse, TUI,
CLI, MCP, and JSON-RPC; compares normalized action/result/frame receipts; and
fails if a required surface lacks a component fallback. Real POG migration work
belongs in the POG repository under its own managed workflow.

## Slices

| # | Slice | Kind | Scope (one line) | Depends on | Status | File |
|---|---|---|---|---|---|---|
| 1 | application-contract | runtime | `application/v1`, canonical frame/action schemas, loader validation, composition, provenance, and legacy typed-view projection | — | not cut | — |
| 2 | operation-registry | runtime + tracing | Typed intent/Starlark operations, effect/capability checks, discovery, common result envelope, and cross-transport receipts | 1 | not cut | — |
| 3 | web-application-runtime | tui + runtime | Vue component registry, default frame renderer, scoped theming, Vite dev/HMR ownership, stale-session rules, and production bundle | 1, 2 | not cut | — |
| 4 | wizard-application-framework | story | Reusable headless + default card presentation with navigation, forms, validation, errors, progress, and extension fixtures | 1, 3 | not cut | — |
| 5 | surface-adapters | tui + runtime | VS Code web/native, TUI, CLI, MCP, and JSON-RPC projections over the shared frame/operation contracts | 1, 2, 4 | not cut | — |
| 6 | POG adoption and conformance | story | Migrate one vertical slice, generate the ownership report, then close every remaining non-story product surface | 3, 4, 5 | external; not cut | — |

## Sequencing

```text
#1 application contract ──▶ #2 operation registry ──▶ #5 surface adapters
          │                         │                         │
          └──────────────▶ #3 web runtime ──▶ #4 wizard framework
                                                     │
                                                     └──────▶ #6 POG adoption
```

Slice 2 and the frame-only portion of slice 3 can proceed in parallel after the
application schema is pinned. The POG slice starts with one vertical workflow
after slices 1–5 have conformance fixtures; full migration follows only after
that slice proves web HMR, VS Code reuse, terminal fallbacks, and transport
parity.
...
1. **The semantic frame is canonical; Vue is optional.** A story application
   ...
   JavaScript.
2. **Workflow and UI share one action boundary.** Components dispatch declared
   intents/operations; they do not acquire a second application controller.
3. **One operation registry feeds every protocol.** JSON-RPC, MCP, CLI, web,
   VS Code, and TUI adapters contain no business behavior.
4. **Web is the reusable rich-presentation substrate.** VS Code reuses it by
   default and adds native projections only where editor-native UX is better.
5. **Custom presentation is trusted application code with explicit
...
6. **Composition is explicit and fail-fast.** Namespace, compatibility,
   fallback, and effect-policy violations fail at load/conformance time.
7. **Durable sessions are never silently hot-reloaded across incompatible
   schemas.** HMR convenience does not override runtime truth.
8. **Tests are deterministic and no-LLM.** Contract matrices, flow cassettes,
...
- Allowing arbitrary browser modules to invoke host functions or filesystem/
  network capabilities outside exported operations.
