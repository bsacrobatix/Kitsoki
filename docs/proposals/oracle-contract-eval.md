# Tracing: Oracle call-site contract tests + correctness eval
...
A kitsoki story's interpretive decisions are oracle call sites — an
`oracle.decide` with a `prompt_path` + `schema` (e.g. the `proposal_brief_judge`
...
2. **No measured confidence per backend.** We have a free local backend
   (`internal/oracle/local_llm.go`, llama.cpp) alongside Claude, but no way to
   ...
   model. `docs/proposals/local-model-oracle.md` wants to route small decides
...
The unifying need: **measure oracle call-site correctness, per backend,
...
- **Layer 1 — offline schema-conformance (free, CI-default).** `kitsoki
  ...
  existing `oracle.ValidateSubmission` (`internal/oracle/validate.go:48`). Same
...
- **Producers:** no new trace events — this *consumes* existing call-site
  metadata (prompt_path / schema / agent on the `Effect.Invoke`) and oracle
  outputs.
- **Consumers:** `kitsoki cassette lint`/`verify` (new), and
  `docs/proposals/local-model-oracle.md`'s routing decision (the eval report is
...
- **Docs on ship:** `docs/tracing/cassettes.md` (Layer 1) + a new
  `docs/testing/oracle-evals.md` (Layer 2).
...
| `judge` | a separate `oracle.decide` rules response ≈ expected | open-ended outputs; **note: the judge is itself a call site that should have its own eval** |
...
The eval runs against any oracle transport already in the tree
(`internal/oracle`: in-process Claude, subprocess, MCP-HTTP, and the local
...
grammar enforcement already in `internal/oracle/grammar_subset.go`, so Layer 1
...
- **Consumer:** `local-model-oracle`'s router consults the report (or a
  ...
  *routing* is local-model-oracle's slice.
...
- [ ] 1.1 cassette lint --app: resolve episode → call-site schema, validate
          response.data.submitted via oracle.ValidateSubmission
...
## 4. Document + hand off
- [ ] 4.1 docs/tracing/cassettes.md (Layer 1); docs/testing/oracle-evals.md (Layer 2)
- [ ] 4.2 Note in local-model-oracle.md how it consumes the report
...
- The routing decision itself (which backend serves a call in production) —
  that's `docs/proposals/local-model-oracle.md`; this proposal only produces the
  evidence.
- New trace event types — this consumes existing call-site metadata and oracle
