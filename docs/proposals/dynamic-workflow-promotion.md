...
- copy the validated story files into `stories/<slug>/` or, with explicit operator approval, `internal/basestories/stories/<slug>/`;
...
Promotion is an operator-reviewed action. It should produce a normal diff that can be tested, reviewed, and committed.
...
| Live LLM/agent decision only | Do not cassette silently; write TODO and link trace turn. |
...
1. Should export create commits automatically? *Lean: no. Produce a normal working-tree diff; separate git-ops can commit it.*
2. Should base-story export rewrite imports and docs automatically? *Lean: only after project-local promotion is proven.*
