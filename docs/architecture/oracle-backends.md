# Oracle Backends (`--oracle claude | copilot | codex`)
...
| `codex` | `codex` on `PATH` | `KITSOKI_ORACLE_CODEX_BIN` |
...
> with the bypass flag so the tool can execute. **Select `--oracle codex` only in
...
- `internal/host/oracle_backend_codex.go` / `internal/host/oracle_stream_codex.go`
...
- `internal/host/oracle_codex_smoke_test.go` — the codex counterpart (same
...
- **Codex MCP execution requires the bypass flag.** Resolved by design, not a
  ...
  tool without `--dangerously-bypass-approvals-and-sandbox`, so `--oracle codex`
