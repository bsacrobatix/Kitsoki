...
| **producer (slidey)** | **the reference plugin** — `~/code/slidey/web/useDeck.js` posts `embed:view` on every scene render; `~/code/slidey/web/embed-annotate.js` owns the element model (keyed off the deck's stable element ids — one place, no per-component tags), draws pick markers over the real rendered elements, and posts `embed:pick`; wired in `web/components/App.vue`. |
