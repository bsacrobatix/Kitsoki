...
`-global-reassign` / `-recursion` (dialect relaxations), `-r` (recurse), `-q`
...
you want to prove *"a `pure` function can't reference `http`"*, run it with a
...
| Static validity + level enforcement | starcheck | `starcheck -predeclared=<level names> -r scripts/` |
