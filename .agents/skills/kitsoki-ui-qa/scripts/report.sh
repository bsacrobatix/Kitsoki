...
shift || true
out="" strict=0 blank_scan="" blank_strict=0 pacing_scan="" pacing_strict=0 rrweb_scan="" rrweb_strict=0 scroll_scan="" scroll_strict=0
while [ $# -gt 0 ]; do
  case "$1" in
    --out)           out="$2"; shift 2 ;;
...
bf="$(mktemp)"; pf="$(mktemp)"; vf="$(mktemp)"; rf="$(mktemp)"; sf="$(mktemp)"
trap 'rm -f "$bf" "$pf" "$vf" "$rf" "$sf"' EXIT
...
gate_pass=1
for n in "$blockers" "$vis_block" "$ann_block" "$blank_block" "$pacing_block" "$rrweb_block" "$scroll_block" "$adv_block"; do
  [ "$n" -eq 0 ] || gate_pass=0
done
