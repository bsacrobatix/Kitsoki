...
mining:
{yaml_dump(data.get("mining_recommendation") or transcript_evidence(Path(data["target_path"])), 2)}
...
def main() -> int:
    ...
    if draft_profile is not None and "mining" not in draft_profile:
        draft_profile["mining"] = data.get("mining_recommendation") or transcript_evidence(Path(data["target_path"]))
