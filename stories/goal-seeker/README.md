...
start → bootstrap(init) → lint_check → evaluate ──remaining==0──▶ retrospective → @exit:done
                                    ...
                                    remaining>0
                                          ▼
   deciding → deciding_gate(decide) → dispatch → redcheck(RED-first) → punch-list
        ▲                                                                   │ done
        └──────────────── evaluate ◀── record(log integration) ◀── ship-it(integrate_existing)
```
...
3. **RED-first** (`flows/red_first_refuses.yaml`). Before any work, `redcheck` runs the
   change's gate on the current tree. A gate that already **passes** (exit 0) is not
   RED — the change refuses to start (`needs_human`) rather than dispatch a no-op.
