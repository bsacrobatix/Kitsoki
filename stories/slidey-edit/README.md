...
| `rendering` | deterministic | `host.slidey.render` (`format: html`, `slidey bundle`) → a self-contained **static HTML deck** and, when available, a `.semantic.json` sidecar. Render failures are classified before any repair agent runs: deck-shaped failures may auto-repair, external renderer/toolchain failures surface directly. `host.artifacts_dir` emits the deck handle and co-locates any sidecar/poster companions. Auto-advances. |
