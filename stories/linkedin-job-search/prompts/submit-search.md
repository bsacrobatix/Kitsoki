...
not append USA or remote to the keyword query. Snapshot the Jobs page. If Remote
is not already selected, use a semantic `click` target for the visible Remote
button or option, then snapshot again to wait for the filtered page. Finish with
exactly one `extract` of visible job-result cards. Return
