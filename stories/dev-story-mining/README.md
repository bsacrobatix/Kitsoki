...
The final `record` room also writes deterministic review artifacts under
`.artifacts/dev-story-mining/<job>/`: `report.md`, `summary.json`, and
`deck.slidey.json`. The deck is generated by
`stories/dev-story-mining/scripts/mining_report.py` from structured phase
artifacts and `tools/report-deck/deterministic_deck.py`; no LLM drafts the
deck.
