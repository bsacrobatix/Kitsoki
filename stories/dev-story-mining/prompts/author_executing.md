...
2. Write a **cassette-free** flow fixture under that story's `flows/` covering each branch — seed the artifacts in `initial_world` so `once:` short-circuits any oracle call (no live LLM, per AGENTS.md). Assert the side effects, not just `next_state`.
