...
def run(project: str, check: bool, timeout: int, generated_at: str = "") -> dict:
    if not generated_at:
    ...
    for target in targets:
        ...
        if not check:
            if target.get("validation_command"):
                row["verify"] = {"status": "skipped", "detail": "pass --check to run heavyweight local oracle", "output": []}
            elif status == "cached_validated":
                row["verify"] = {"status": "cached", "detail": "pass --check with required local repo env to re-run", "output": []}
            else:
                row["verify"] = {"status": "skipped", "detail": "summary mode", "output": []}
            rows.append(row)
            continue
        ...
        if validation_command:
            try:
                ...
                output = []
                if exc.stdout:
                    output.extend(output_lines(str(exc.stdout)))
                if exc.stderr:
                    output.extend(output_lines(str(exc.stderr)))
                row["verify"] = {"status": "timeout", "detail": f"after {timeout}s", "output": output}
                rows.append(row)
                continue
            if result.returncode == 0:
...
def main() -> None:
    ...
    summary = run(args.project, check=args.check, timeout=args.timeout, generated_at=args.generated_at)
    print("Story QA")
    for target in summary["targets"]:
