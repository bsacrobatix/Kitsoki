...
def story_summary(name: str, sc: StoryCost, bl: Baseline) -> dict:
    baseline = {
        "ops_sampled": bl.total_n(),
        "sessions_matched": bl.sessions_matched,
        "transcripts": bl.transcripts,
        "exact_priced": bl.exact,
        "reprocessing_tokens": _reproc_share(bl),
    }
    if bl.total_n():
        baseline.update({
            "median_usd": _median(bl.all_op_costs),
            "p90_usd": _pct(bl.all_op_costs, 0.9),
            "max_usd": max(bl.all_op_costs),
            "models": sorted({m for st in bl.ops.values() for m in st.models}),
        })
    rewarms = [r for st in bl.ops.values() for r in st.rewarm_usd]
    if rewarms:
        baseline["cold_resumes"] = len(rewarms)
        baseline["median_rewarm_usd"] = _median(rewarms)
    intents = []
    for op, st in sorted(bl.ops.items(), key=lambda r: -_median(r[1].costs)):
        if st.n() == 0:
            continue
        intents.append({
            "operation": op,
            "n": st.n(),
            "median_usd": _median(st.costs),
            "p90_usd": _pct(st.costs, 0.9),
    ...
    out = {
        ...
        "recorded": sc.recorded,
        "story_cost_usd": sc.usd if sc.measured else None,
        "agent_calls": sc.agent_calls,
        "models": sorted(sc.models),
        "breakdown": [{"handler": h, "cost_usd": c} for h, c in sc.breakdown],
        "cassettes": sc.cassettes,
        "baseline": baseline,
        "intents": intents,
    }
    if sc.measured and bl.total_n():
        out["savings_median_usd"] = _median(bl.all_op_costs) - sc.usd
        out["savings_ratio_median"] = (_median(bl.all_op_costs) / sc.usd) if sc.usd > 0 else None
    return out
...
def compute(stories: list, project_globs: list) -> list:
    computed = []
    for name in stories:
...
def build_summary_from_computed(computed: list, project_globs: list, markdown_path: str = "") -> dict:
    return {
        "tool": "tools/session-mining/cost_report.py",
        "project_globs": project_globs,
        "markdown_path": markdown_path,
        "stories": [story_summary(name, sc, bl) for name, sc, bl in computed],
    }


def build_summary(stories: list, project_globs: list, markdown_path: str = "") -> dict:
    return build_summary_from_computed(compute(stories, project_globs), project_globs, markdown_path)


def render_report(computed: list) -> str:
    L = []
...
def build_report(stories: list, project_globs: list) -> str:
    return render_report(compute(stories, project_globs))
...
def write_slidey_spec(path: str, summary: dict) -> None:
    builder = os.path.join(REPO_ROOT, "tools", "report-deck", "deterministic_deck.py")
    subprocess.run(
        [
            sys.executable,
            builder,
...
def main(argv=None) -> int:
    ...
    computed = compute(stories, project_globs)
    md = render_report(computed)
    if args.out:
        ...
        sys.stdout.write(md)
    if args.summary or args.slidey_spec:
        summary = build_summary_from_computed(computed, project_globs, markdown_path=args.out or "")
        if args.summary:
