...
project=""; bug=""; cand=""; repo_dir=""; completion_state=""; do_score=0; no_drive=0; keep_workspace=0; orch="${MCP_DRIVE_MODEL:-gpt-5.5}"; use_docker_score="${EXTERNAL_BAKEOFF_USE_DOCKER_SCORE:-1}"; story="stories/bench-bugfix/app.yaml"
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -n "$project" && -n "$bug" && -n "$cand" ]] || {
  echo "usage: drive_cell.sh --project <name> --bug <id> --candidate <key> [--repo-dir <local-checkout>] [--score] [--no-drive] [--no-docker-score] [--keep-workspace] [--completion-state <path>] [--story <repo-relative app.yaml>]" >&2; exit 2; }
