...
/** Token usage + cost for a single oracle.call.complete event. */
...
/**
 * Read token usage + cost from an oracle.call.complete event's attrs.
 *
 * The canonical shape the engine emits is the opaque transport `meta`:
 *   attrs.meta = { usage: { input_tokens, output_tokens,
 *                           cache_read_input_tokens, cache_creation_input_tokens },
 *                  cost_usd }
 * (the raw claude-CLI usage object — see docs ORACLE_ATTRS.md). We fall back to
 ...
 */
export function readOracleUsage(attrs: Record<string, unknown> | undefined): OracleUsage {
  const a = attrs ?? {};
