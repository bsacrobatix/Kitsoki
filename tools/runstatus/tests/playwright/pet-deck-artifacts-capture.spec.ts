/**
 * pet-deck-artifacts-capture.spec.ts — rrweb capture of each PRODUCED slidey deck
 * for the trace-column pet scenario, played as a clean standalone deck.
 *
 * The scenario produces six self-contained slidey decks (HTML). The deck-review
 * clips (pet-<phase>-review) capture them through kitsoki's review surface — the
 * deck sits in a sandboxed iframe inside the chat/trace chrome, so the clip is
 * really a screenshot of kitsoki, not the deck. This spec instead opens the
 * RENDERED deck HTML at top level (no iframe, no sandbox) and steps through its
 * slides, so rrweb records the deck's own slide/reveal DOM transitions — a clean
 * "here is the produced deck" artifact video.
 *
...
const PHASES = ["prd", "design", "decomposition", "bug-report", "bugfix-report", "feature-demo"] as const;
