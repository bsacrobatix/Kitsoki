...
import { readOracleUsage } from "../../src/components/oracle/lib.js";

describe("readOracleUsage", () => {
  it("reads the canonical claude-CLI meta.usage shape", () => {
    const u = readOracleUsage({
      verb: "task",
      meta: {
  ...
  it("returns all-undefined for an event with no usage", () => {
    const u = readOracleUsage({ verb: "ask" });
  ...
  it("tolerates undefined attrs", () => {
    expect(() => readOracleUsage(undefined)).not.toThrow();
  });
});
