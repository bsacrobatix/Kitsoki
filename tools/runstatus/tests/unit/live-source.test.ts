...
describe("LiveSource", () => {
  ...
  it("listArtifactJobs calls runstatus.jobs.list", async () => {
    fetchMock.mockResolvedValueOnce(rpcOk([{ job_id: "job-1", run_url: "/s/job-1" }]));
    const src = new LiveSource("/");
    const jobs = await src.listArtifactJobs();
