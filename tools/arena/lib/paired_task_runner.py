...
def build_kitsoki_prompt(
    ...
    Tool names below use kitsoki's own dotted MCP names (studio.ping,
    session.new, ...) because this prompt only ever runs under the CODEX
    orchestrator (dispatch_kitsoki always sets MCP_DRIVE_BACKEND=codex).
    drive_cell.sh's original template writes underscored names
    (studio_ping/session_new) because IT drives via the CLAUDE backend, which
    client-side-renames dotted MCP tool names to mcp__kitsoki__session_new;
    codex's tool_search indexes the server's raw dotted name, so reusing the
    underscored spelling here made every tool_search query return 0 results."""
