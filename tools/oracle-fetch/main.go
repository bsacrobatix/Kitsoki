// Command oracle-fetch pre-warms the local-model oracle cache: it downloads and
...
// box never hits a multi-GB fetch on the first oracle.local call. It is the
...
// Usage:
//
//	go run ./tools/oracle-fetch -binary            # fetch the llama-server binary
//	go run ./tools/oracle-fetch -model <id>        # fetch one model's weights (default: Qwen2.5-1.5B)
//	go run ./tools/oracle-fetch -binary -model <id>
//
...
import (
	...
	"kitsoki/internal/oracle/server"
)
