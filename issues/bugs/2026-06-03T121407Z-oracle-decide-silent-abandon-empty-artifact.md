---
id: 2026-06-03T121407Z-oracle-decide-silent-abandon-empty-artifact
title: "oracle.decide with no submit routes to success with an empty artifact instead of failing visibly"
target: kitsoki
...
external: {}
assignee: ""
url: "issues/bugs/2026-06-03T121407Z-oracle-decide-silent-abandon-empty-artifact.md"
---
...
it — `oracle-off-ramp.md` is adjacent but solves a different problem (no-*match*
→ converse, not empty-*submit* → success).

When the LLM behind a `host.oracle.decide` answers in prose (or returns an
...
- Isolate oracle subagents from operator-global Claude plugins
...
- `internal/orchestrator/` — oracle `decide` result handling / gate routing
  on empty submit.
